...
import { ImageIcon, Hash, FileText, CheckCircle2 } from 'lucide-react';
...
/**
 * StatsBar — shows REAL stats computed from the user's actual history only.
 * No fake/baseline numbers whatsoever.
 *
 * Props:
 *   history {Array} — the full history array from App state
 */
const StatsBar = ({ history = [] }) => {
  ...
  const STATS = [
    { Icon: ImageIcon,    value: imagesAnalyzed,     label: 'Images Analyzed',    color: 'text-brand-cyan',         iconBg: 'bg-brand-cyan/10 border-brand-cyan/20' },
    { Icon: Hash,         value: keywordsGenerated,  label: 'Keywords Generated',  color: 'text-brand-violet-light', iconBg: 'bg-brand-violet/10 border-brand-violet/20' },
    { Icon: FileText,     value: blogPostsCreated,   label: 'Blog Posts Created',  color: 'text-emerald-400',        iconBg: 'bg-emerald-500/10 border-emerald-500/20' },
    { Icon: CheckCircle2, value: successfulAnalyses, label: 'Successful Analyses', color: 'text-amber-400',          iconBg: 'bg-amber-500/10 border-amber-500/20' },
  ];

  // First-time visitor — no history yet, show a gentle hint
  if (imagesAnalyzed === 0) {
    return (
      <motion.div
        ...
        className="w-full max-w-2xl mx-auto mt-8 mb-2"
      >
        <p className="text-center text-xs text-gray-600 tracking-wide">
          📊 Your personal stats will appear here after your first analysis
        </p>
      </motion.div>
    );
  }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {STATS.map(({ Icon, value, label, color, iconBg }) => (
          <motion.div
            ...
            className="glass rounded-2xl p-4 flex flex-col items-center text-center gap-2 border border-white/5 hover:border-white/10 transition-all cursor-default"
          >
